...
class Busca(object):
    ...
    def up(self, pai, i):
        new = Node(pai.estado, 'up', pai, pai.custo + 1, pai.profundidade + 1)
        new.estado[i] = new.estado[i + 3]
        new.estado[i + 3] = 0
        return new

    def down(self, pai, i):
        new = Node(pai.estado, 'down', pai, pai.custo + 1, pai.profundidade + 1)
        new.estado[i] = new.estado[i - 3]
        new.estado[i - 3] = 0
        return new

    def left(self, pai, i):
        new = Node(pai.estado, 'left', pai, pai.custo + 1, pai.profundidade + 1)
        new.estado[i] = new.estado[i + 1]
        new.estado[i + 1] = 0
        return new

    def right(self, pai, i):
        new = Node(pai.estado, 'right', pai, pai.custo + 1, pai.profundidade + 1)
        new.estado[i] = new.estado[i - 1]
        new.estado[i - 1] = 0
        return new
